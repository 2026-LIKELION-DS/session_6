...
function dbcomentBox() {
    ...
    div.innerHTML = `
        <input type="text" class="comentbox" placeholder="대댓글을 입력하세요">
        <ul class="textfunction">
            <li class="check-coment">
                <input type="checkbox" class="anonymousBox">익명
            </li>
...
// "대댓글" 버튼 클릭 → 입력창 생성
document.addEventListener("click", function (e) {
    const replyBtn = e.target.closest(".jscoment");
    if (!replyBtn) return;

    const commentBox = replyBtn.closest(".sectionComment");
...
// 대댓글 작성 버튼 클릭 (지금은 테스트용 alert)
document.addEventListener("click", function (e) {
    const reWriteBtn = e.target.closest(".reWriteBtn");
    if (!reWriteBtn) return;

    const comentContainer = reWriteBtn.closest(".comentContainer");
    const input = comentContainer.querySelector(".comentbox");
